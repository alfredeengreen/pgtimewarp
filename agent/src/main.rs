...
use store::{checkpoints, StoreManager};
...
async fn run_agent(
    ...
    let mut tracking_refresh = tokio::time::interval(std::time::Duration::from_secs(
        config.intervals.refresh_tracked_s,
    ));
    let mut node_heartbeat = tokio::time::interval(std::time::Duration::from_secs(30));
    let mut lsn_time_map_interval = tokio::time::interval(std::time::Duration::from_secs(5));
